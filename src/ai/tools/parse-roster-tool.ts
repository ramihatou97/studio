...
export const parseRosterTool = ai.defineTool(
    ...
    async (input) => {
        // This internal prompt is highly specialized for parsing rosters.
        const parseRosterPrompt = ai.definePrompt({
            ...
            **CRITICAL INSTRUCTIONS:**
            1.  **Identify Sections**: The document may have sections like "On service residents" and "Off service residents" or "Fly-In Residents". You must identify which section each person belongs to.
        ...
        const { output } = await parseRosterPrompt(input);
        return output!;
    }
);
